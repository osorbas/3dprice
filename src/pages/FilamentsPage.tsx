...
import { Trash2, Package, LayoutGrid, List, Filter } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
const FilamentsPage = () => {
  ...
  const filteredFilaments = useMemo(() => {
    ...
    if (selectedBrandFilter === "all") return sorted;
    return sorted.filter((f) => f.brand === selectedBrandFilter);
  }, [filaments, selectedBrandFilter]);
      ...
      {filaments.length === 0 ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">Filtrar por Marca:</span>
          </div>
          <ScrollArea className="w-full whitespace-nowrap pb-4">
            <div className="flex w-max space-x-2">
              {brands.map((brand) => (
                <Badge
                  key={brand}
                  variant={selectedBrandFilter === brand ? "default" : "outline"}
                  className={cn(
                    "cursor-pointer px-3 py-1 text-sm transition-colors",
                    selectedBrandFilter !== brand && "hover:bg-accent"
                  )}
